...
export class CreateClinicalEntryUseCase {
  constructor(
    private readonly clinicalEntryPort: ClinicalEntryRepositoryPort,
  ...
  async execute(input: CreateClinicalEntryDto): Promise<ClinicalEntry> {
    ...
    if (!history)
      throw new Error('history not found, impossible add a clinical entry');
    if (!history.status)
      throw new Error('History is archived, cannot add clinical entries');
    const date = new Date();
