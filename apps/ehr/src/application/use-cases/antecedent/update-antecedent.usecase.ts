...
import { AntecedentUpdatedEvent } from 'apps/ehr/src/domain/events';
...
export class UpdateAntecedentUseCase {
  ...
  async execute(
    ...
    const updatedAntecedent = await this.antecedentPort.update(changes);

    // Publish event
    const event = new AntecedentUpdatedEvent(
      updatedAntecedent.antecedentId!,
      updatedAntecedent.historyId,
      input.type,
      input.description,
      {
        timestamp: new Date(),
